...
public class DeadlockTest extends TestUtil.CreateHeapFile {
    ...
    public TestUtil.LockGrabber startGrabber(TransactionId tid, PageId pid,
        ...
        TestUtil.LockGrabber lg = new TestUtil.LockGrabber(tid, pid, perm);
        lg.start();
        return lg;
    }
    ...
    public void testReadWriteDeadlock() throws Exception {
        ...
        TestUtil.LockGrabber lg1Read = startGrabber(tid1, p0, Permissions.READ_ONLY);
        TestUtil.LockGrabber lg2Read = startGrabber(tid2, p1, Permissions.READ_ONLY);

        // allow read locks to acquire
        Thread.sleep(POLL_INTERVAL);

        TestUtil.LockGrabber lg1Write = startGrabber(tid1, p1, Permissions.READ_WRITE);
        TestUtil.LockGrabber lg2Write = startGrabber(tid2, p0, Permissions.READ_WRITE);
    ...
    public void testWriteWriteDeadlock() throws Exception {
        ...
        TestUtil.LockGrabber lg1Write0 = startGrabber(tid1, p0, Permissions.READ_WRITE);
        TestUtil.LockGrabber lg2Write1 = startGrabber(tid2, p1, Permissions.READ_WRITE);

        // allow initial write locks to acquire
        Thread.sleep(POLL_INTERVAL);

        TestUtil.LockGrabber lg1Write1 = startGrabber(tid1, p1, Permissions.READ_WRITE);
        TestUtil.LockGrabber lg2Write0 = startGrabber(tid2, p0, Permissions.READ_WRITE);
    ...
    public void testUpgradeWriteDeadlock() throws Exception {
        ...
        TestUtil.LockGrabber lg1Read = startGrabber(tid1, p0, Permissions.READ_ONLY);
        TestUtil.LockGrabber lg2Read = startGrabber(tid2, p0, Permissions.READ_ONLY);

        // allow read locks to acquire
        Thread.sleep(POLL_INTERVAL);

        TestUtil.LockGrabber lg1Write = startGrabber(tid1, p0, Permissions.READ_WRITE);
        TestUtil.LockGrabber lg2Write = startGrabber(tid2, p0, Permissions.READ_WRITE);
